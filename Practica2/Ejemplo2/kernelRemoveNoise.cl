...
void md5(int *h0, int *h1, int *h2, int *h3, 
	char *initial_msg, int initial_len) {
 
    // Message (to prepare)
    char msg[MAX_DIGIT_LEN+64+56];
...
__kernel void kernelRemoveNoise(__global char * msg_cracked, int nchars, __global char * cset, int h0_md5, int h1_md5, int h2_md5, int h3_md5,  __global int * basis){
    ...
    if(i < nchars && j < basis[i]){
        gen_msg(msg, i, cset, basis, j);
